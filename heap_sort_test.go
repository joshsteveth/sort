...
func TestNewHeap(t *testing.T) {
	...
	assert.Equal(t, 6, h.leftChild.value)
	assert.Equal(t, 7, h.rightChild.value)
	assert.Equal(t, 4, h.leftChild.leftChild.value)
	assert.Equal(t, 5, h.leftChild.rightChild.value)
	assert.Equal(t, 3, h.rightChild.leftChild.value)
	assert.Equal(t, 2, h.rightChild.rightChild.value)
	assert.Equal(t, 1, h.leftChild.leftChild.leftChild.value)
}
...
func TestHeapSort(t *testing.T) {
	...
	foo = HeapSort(foo)
	expected := []int{8, 7, 6, 5, 5, 4, 4, 3, 2, 1}
